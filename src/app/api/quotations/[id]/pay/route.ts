...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${LARAVEL_API_URL}/quotations/${encodeURIComponent(id)}/pay`, {
      method: "POST",
    ...
    if (res.ok) {
      try {
        ...
        if (quoteRes.ok) {
          ...
          if (quotation) {
            const verifyPaymentUrl = `${process.env.NEXT_PUBLIC_APP_URL || process.env.APP_URL || "http://localhost:3000"}/api/quotations/${encodeURIComponent(id)}/payment-approved?source=payment-verification`;
