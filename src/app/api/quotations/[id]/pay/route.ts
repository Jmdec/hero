    ...
    if (res.ok) {
      try {
        ...
        if (quoteRes.ok) {
          ...
          if (quotation) {
            const verifyPaymentUrl = `${process.env.NEXT_PUBLIC_APP_URL || process.env.APP_URL || "http://localhost:8000"}/api/quotations/${encodeURIComponent(id)}/payment-approved?source=payment-verification`;
