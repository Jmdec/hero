...
function buildInternalInquiryHtml(payload: ContactInquiryPayload, adminUrl: string, replyUrl: string) {
  return `
...
function buildJapaneseInternalInquiryHtml(payload: ContactInquiryPayload, adminUrl: string) {
  return `
    <div style="font-family:Arial,Helvetica,sans-serif;background:#f4f7fb;padding:32px 20px;">
      <div style="max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #e8edf5;border-radius:16px;overflow:hidden;">
        <div style="height:6px;background:#0D47A1;"></div>
        <div style="padding:28px 32px;">
          ...
          <table style="width:100%;border-collapse:collapse;">${buildRows(payload)}</table>
          <div style="margin-top:18px;padding:16px;border-radius:12px;background:#f8fafc;border:1px solid #e2e8f0;">
            <p style="margin:0 0 8px;font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#64748b;">Message</p>
            <p style="margin:0;color:#1e293b;font-size:14px;line-height:1.7;">${payload.message.replace(/\n/g, "<br/>")}</p>
          </div>
...
async function sendInquiryNotifications(payload: ContactInquiryPayload, origin: string) {
  ...
  const adminUrl = `${origin}/admin/contact`;
  // NOTE: the recipient address in a mailto: link must NOT be URI-encoded.
  // Encoding it (e.g. with encodeURIComponent) turns "@" into "%40", which
  // many mail clients fail to parse as a valid "To" address, leaving the
  // field blank when the link is opened. Only the query string params
  // (subject, body, etc.) should be encoded.
  const replySubject = `Re: Your ${getContactInquiryLabel(payload.inquiryType)} inquiry`;
  // Quote the original inquiry like a normal email reply, so the mail
  // client opens pre-filled with context instead of a blank compose window.
  // (mailto: can't do true In-Reply-To/References threading since that
  // requires the mail client to already have the original message stored —
  // this mimics the visual/textual shape of a reply instead.)
  ...
  const html = buildInternalInquiryHtml(payload, adminUrl, replyUrl);
  const japaneseHtml = buildJapaneseInternalInquiryHtml(payload, adminUrl);
  ...
  for (const recipient of [recipients.president, recipients.chairman]) {
    ...
    tasks.push(sendMail({
      to: recipient,
      subject: `【新規お問い合わせ】${payload.name} / ${getContactInquiryLabel(payload.inquiryType)}`,
      html: japaneseHtml,
      text,
      replyTo: payload.email,
    }));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (response.ok) {
      const saved = (data?.data ?? {}) as { id?: number; dynamic_data?: Record<string, string> };
