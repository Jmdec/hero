  ...
  return (
    <main className="min-h-screen">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-4 space-y-6">
        ...
        {/* Responsive listing: cards on mobile/tablet, table on desktop */}
          ...
          {!loading && !error && items.length === 0 && (
            <div className="rounded-xl border border-slate-200 bg-white px-5 py-14 text-center shadow-sm">
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm font-medium text-slate-700">No testimonials found</p>
          ...
          {!loading && !error && items.length > 0 && (
            <>
              <div className="grid gap-3 lg:hidden">
                {items.map((t) => {
                  const submittedDate = formatDate(t.created_at) ?? "-";
                  const personTitle = normalizeNullableField(t.title);
                  const personCompany = normalizeNullableField(t.company);
                  const serviceType = normalizeNullableField(t.service_type) ?? "-";
                  const ratingText = `${t.rating}/5`;
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span
                          className={`shrink-0 inline-flex items-center rounded-full px-3 py-1 text-[11px] font-medium capitalize ${STATUS_STYLES[t.status] ?? "bg-slate-100 text-slate-600 ring-1 ring-inset ring-slate-200"}`}
                        >
                          {t.status}
                        </span>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                        <div>
                          ...
                          <p className="mt-0.5 font-medium text-slate-700">{serviceType}</p>
                        </div>
                        <div>
                          <p className="text-slate-400">Submitted</p>
                          <p className="mt-0.5 font-medium text-slate-700">{submittedDate}</p>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-slate-600">{ratingText}</span>
                      </div>

                      <div className="mt-4 flex flex-wrap gap-2">
                        <button
                          onClick={() => openView(t)}
                          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-600"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:border-red-200 hover:bg-red-50 hover:text-red-600"
                        >
                        ...
                        <tr key={t.id} className="hover:bg-blue-50/40">
                          <td className="px-5 py-4 align-top">
                            ...
                            <p className="line-clamp-1 text-xs text-slate-500">
                              ...
                              {normalizeNullableField(t.company)
                                ? ` · ${normalizeNullableField(t.company)}`
                                : ""}
                            </p>
                          </td>
                          ...
                          <td className="px-5 py-4 align-top text-slate-700">
                            {normalizeNullableField(t.service_type) ?? "-"}
                          </td>
